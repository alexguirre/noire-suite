...
namespace noire::trunk
{
	...
	static void RegisterDeclarations(std::unordered_map<u32, VertexDeclaration>& decls)
	{
		...
		// clang-format off
		decls.emplace(0xAAAAAAAA, Decl{ { // used by noire-file-explorer Renderer
			{ 0, 0x00, DXGI_FORMAT_R32G32B32_FLOAT,		S::POSITION },
			{ 0, 0x0C, DXGI_FORMAT_R32G32B32A32_FLOAT,	S::COLOR },
		} });
	...
	{
		Expects(std::is_sorted(elements.begin(), elements.end(), [](auto& a, auto& b) {
	...
	{
		VertexDeclaration& d = mDeclarations.at(id);
		if (!d.InputLayout)
		{
